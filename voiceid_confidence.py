...
def create_synthetic_voice_samples(
    user_id: str, base_freq: float = 220.0, duration: float = 15.0
):
    """
    ...
    # Create 15 different voice samples for enrollment
    samples = []
    for i in range(15):
...
def test_confidence_and_discrimination():
    ...
    obama_samples = create_synthetic_voice_samples(
        "obama", base_freq=150.0, duration=10.0
    )
    jfk_samples = create_synthetic_voice_samples("jfk", base_freq=180.0, duration=10.0)
    unknown_samples = create_synthetic_voice_samples(
        "unknown", base_freq=200.0, duration=10.0
    )
    ...
    obama_success = processor.enroll_user(
        "obama", obama_samples[:8]
    )  # Use 8 samples for enrollment
    jfk_success = processor.enroll_user("jfk", jfk_samples[:8])
    ...
    # Test Obama samples
    obama_confidences = []
    for i, sample in enumerate(obama_samples[8:]):  # Use unused samples
    ...
    # Test JFK samples
    jfk_confidences = []
    for i, sample in enumerate(jfk_samples[8:]):
        speaker, confidence = processor.identify_speaker(sample)
